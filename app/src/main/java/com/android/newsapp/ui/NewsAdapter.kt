...
class NewsAdapter(private val onBookmarkClick : (NewsEntity) -> Unit): ListAdapter<NewsEntity, MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bind(news)
        val isBookmark = holder.binding.ivBookamrk
        if (news.isBookmarked) {
            isBookmark.setImageDrawable(ContextCompat.getDrawable(isBookmark.context, R.drawable.ic_bookmarked_white))
        } else {
            isBookmark.setImageDrawable(ContextCompat.getDrawable(isBookmark.context, R.drawable.ic_bookmark_white))
        }
        isBookmark.setOnClickListener { 
