...
class NewsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (tabName == TAB_NEWS) {
            viewModel.getHeadlineNews().observe(viewLifecycleOwner) { result ->
                if (result!= null) {
                    when (result) {
                        ...
                        is Result.Error -> {
                            binding?.progressBar?.visibility = View.GONE
                            Toast.makeText(context, "Terjadi Kesalahan" + result.error, Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
                }
            ...
            viewModel.getBookmarked().observe(viewLifecycleOwner) { bookmarked ->
                binding?.progressBar?.visibility = View.GONE
                newsAdapter.submitList(bookmarked)
            }
        }
